...
export class AccountController {
  ...
  public static async login(req: NextApiRequest, res: NextApiResponse) {
    ...
    const token = sign({ role: account.role }, process.env.JWT_SECRET || '');

    res.setHeader('Set-Cookie', serialize(COOKIES_NAME.TOKEN, token, {
      httpOnly: true,
  ...
  public static async logout(req: NextApiRequest, res: NextApiResponse) {
    res.setHeader('Set-Cookie', serialize(COOKIES_NAME.TOKEN, '', {
      httpOnly: true,
      secure: process.env.NODE_ENV === 'production',
      sameSite: 'strict',
      expires: new Date(0),
